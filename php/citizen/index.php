...
<head>
  ...
  <script src="../../js/citizen/demands.js"></script>
...
<body class="bg-gray-900 text-white flex">
  ...
  <div class="flex-1 p-6 ml-64 overflow-y-auto">
    <!-- Modals -->
    <div id="demand-modal" class="modal bg-gray-800 p-6 rounded-lg shadow-lg text-white max-w-xl mx-auto" style="background-color: #1F2937 !important; color: #F9FAFB !important;">
      <div class="modal-wrapper">
        <div class="task-title">Create a Demand</div>
        <div class="task-grid">
          Product: <div id="product-name" class="name"></div>
          Quantity: <input id="product-quantity" type="number" min="1" class="bg-gray-700 text-white rounded p-2 w-full">
        </div>
        <button id="create-demand" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded">Create Demand</button>
      </div>
    </div>

    <div class="wrapper p-6">
      <h1 class="text-3xl font-bold mb-4">Create Demand</h1>
      <div class="flex justify-between items-center mb-4">
        <div class="flex items-center space-x-4">
          <label for="categories-demand" class="text-yellow-600 font-bold">Filter by Category:</label>
          <select id="categories-demand" class="bg-gray-700 text-white rounded p-2"></select>
        </div>
        <div>
          <input id="autocomplete-demand" class="bg-gray-700 text-white rounded p-2 w-full" placeholder="Search products...">
        </div>
      </div>
      <div id="products-grid-demand" class="product-grid grid grid-cols-1 gap-4"></div>
    </div>
